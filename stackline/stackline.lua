...
function stackline:setupListeners() -- {{{
	...
	self.wf:subscribe(self.events.redrawOn, self.redrawWinIndicator)
end -- }}}
...
function stackline.redrawWinIndicator(hsWin, _app, _evt) -- {{{
	...
	if not stackedWin then
		return
	end -- if non-existent, the focused win is not stacked
